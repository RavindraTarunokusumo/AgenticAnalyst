...
- `domain/models.py`: pure Pydantic contracts (Source, Article, BatchSummary, Brief, NarrativeStateVersion, WorkflowRun, WorkflowStatus, ...). No infrastructure imports. `ExtractorKind` (a plain `String(32)` column, no DB check constraint) gained `FILE_PDF`/`FILE_TEXT` members for uploaded-file ingestion - additive, no migration.
...
- `frontend/`: Vite + React + TypeScript + Tailwind CSS v4 (via `@tailwindcss/vite`, no `tailwind.config.js`) SPA, the repo's first Node/npm toolchain. `npm run build` outputs to `frontend/dist/` (gitignored); Vite's `base` is `/ui/` to match the backend mount path. `npm run dev`'s dev-server proxy (`vite.config.ts`) forwards `/briefs`, `/sources`, and `/ingestion` to `http://localhost:8000` - every path prefix `src/api.ts` fetches. `src/api.ts` holds typed fetch wrappers mirroring the backend's Pydantic response models field-for-field (`fetchBriefList`/`fetchBriefDetail`, `fetchSources`/`registerSource`, `ingestUrls`/`ingestFile`/`fetchIngestionAttempts`); all throw on any non-2xx response rather than swallowing errors, and every write wrapper sets `X-API-Key`. `src/components/` holds presentational components with no fetch calls of their own (`CadenceTabs`, `BriefList`, `BriefDetail`, `LoadingState`, `EmptyState`, `ErrorState`, plus the onboarding/ingestion set below); `src/App.tsx` owns all state via plain `useState` - no client-side routing library, no global store (YAGNI for this app's size). `BriefDetail` renders `content` only via JSX text interpolation (a `<pre>` text node), never `innerHTML`/`dangerouslySetInnerHTML`, since brief content is untrusted LLM-generated text.
- First-run onboarding and content ingestion (functionality-only refinement; no visual redesign): `App.tsx` calls `GET /sources` on mount and renders `<Onboarding>` in place of the normal 3-panel view when the result is `[]`. `Onboarding.tsx` registers one default source (`POST /sources`, `stable_id` = the normalized domain the user enters, doubling as a unique-ish slug since the form has no separate stable_id field) plus an optional feed, and persists the API key the user types into `localStorage['ae_api_key']`. Once at least one source exists, `AddContentPanel.tsx` (three modes: paste links, register a feed, upload a file) drives `ingestUrls`/`registerSource`/`ingestFile` and renders per-item results via `IngestionResultList.tsx`; a **thrown** wrapper error (network failure, or a 401/403 from `_require_key` rejecting before any `IngestionResultResponse` exists) is shown as a single inline "check your API key" panel error, distinct from a **successful** response whose items include per-URL/file `status: "failed"` entries. `IngestionResultList` falls back to a locally-known filename when displaying an uploaded file's synthetic `upload://<hash>` `candidate_url`. `RecentActivityList.tsx` (owned by `App.tsx`, refetched via `GET /ingestion/attempts` after every `AddContentPanel` submission) shows ingestion history independent of any single panel session. `ApiKeySettings.tsx` lets the key be viewed/replaced after onboarding, writing to the same `localStorage` key. The API key is client-held in `localStorage` by deliberate design (no login/session system exists in this slice) - not a gap to harden later.
