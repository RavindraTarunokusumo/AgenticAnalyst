...
import { ingestFile, ingestUrls } from '../api'
import { ErrorState } from './ErrorState'
import { IngestionResultList } from './IngestionResultList'

type Mode = 'links' | 'file'
...
export function AddContentPanel({ apiKey, topicId, onSubmitted }: AddContentPanelProps) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      if (mode === 'links') {
        setResults(await ingestUrls(apiKey, topicId, urls))
      } else if (file !== null) {
        const result = await ingestFile(apiKey, topicId, file)
        setResults([result])
