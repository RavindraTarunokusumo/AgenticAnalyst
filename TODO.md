...
- [ ] Task 3 — Implement durable domain and persistence foundations.
  - [ ] 3.1 Add runtime/dev dependencies (SQLAlchemy, Alembic, asyncpg, pgvector, langgraph, langgraph-checkpoint-postgres, testcontainers) and update lock
  - [ ] 3.2 Create Pydantic domain models (src/analyst_engine/domain/models.py): Source, Article, ArticleBatch, BatchSummary, Brief, NarrativeStateVersion, PredictionExpectation, Embedding, WorkflowRun, Cadence, Citation, etc.
  - [ ] 3.3 Bootstrap Alembic (alembic.ini + alembic/env.py + script) wired to our async engine and models
